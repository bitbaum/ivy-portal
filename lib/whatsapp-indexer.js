...
class MessageDB {
  ...
  stats() {
    const chats = this.db.prepare('SELECT COUNT(*) as count FROM chats WHERE channel = ?').get('whatsapp');
    const messages = this.db.prepare('SELECT COUNT(*) as count FROM messages WHERE channel = ?').get('whatsapp');
...
async function indexWhatsAppHistory() {
  ...
  client.on('ready', async () => {
    ...
    try {
      ...
      console.log('[whatsapp-indexer] Indexing: DMs + small groups (<50 members). Skipping: broadcast lists + large groups.');
        ...
        try {
          ...
          for (const msg of messages) {
            ...
            const inserted = db.insertMessage(
              chatFk, 'whatsapp', messageId, senderId, senderName,
              text, timestamp, hasMedia, mediaType !== 'chat' ? mediaType : null
            );
      ...
      console.log(`[whatsapp-indexer] Indexed: ${stats.chats} chats, ${stats.messages} messages (+${totalNewMessages} new)`);
      console.log(`[whatsapp-indexer] Skipped: ${skippedCount} chats (broadcast lists + large groups)`);
