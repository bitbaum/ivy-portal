...
async function indexTelegramHistory() {
  ...
  for (const [index, dialog] of dialogs.entries()) {
    ...
    // Get last indexed message ID (for incremental updates)
    const lastMessageId = db.getLastMessageId('telegram', chatId);
    ...
    try {
      ...
      for (const msg of messages) {
        ...
        const senderName = msg.sender?.firstName || msg.sender?.username || 'Unknown';
