...
function loadConfig() {
  if (!fs.existsSync(CONFIG_PATH)) {
    ...
    console.log(
      '[telegram-indexer] Format: {"apiId": 12345, "apiHash": "abc...", "phoneNumber": "+1234567890"}',
    );
    process.exit(1);
  }
...
async function indexTelegramHistory() {
  ...
  console.log(
    '[telegram-indexer] Indexing: DMs + small groups (<50 members). Skipping: channels + large groups.',
  );
  ...
  for (const [index, dialog] of dialogs.entries()) {
    ...
    try {
      ...
      for (const msg of messages) {
        ...
        const sender = /** @type {{ firstName?: string, username?: string } | undefined} */ (
          msg.sender
        );
  ...
  console.log(
    `[telegram-indexer] Indexed: ${stats.chats} chats, ${stats.messages} messages (+${totalNewMessages} new)`,
  );
