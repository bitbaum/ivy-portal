...
function runShell(command, opts = {}) {
  return new Promise((resolve) => {
    ...
    exec(
      command,
      { timeout, shell: '/bin/bash', env: { ...process.env, PATH, HOME } },
      (err, stdout, stderr) => {
...
app.get('/api/finance', async (_req, res) => {
  ...
  const [upcomingResult, overdueResult] = await Promise.all([
    runShell(`python3 ${WORKSPACE}/tools/recurring-payments.py --upcoming 14 --json`, {
      timeout: 20000,
    }),
    runShell(`python3 ${WORKSPACE}/tools/recurring-payments.py --overdue --json`, {
      timeout: 20000,
    }),
  ]);
  ...
  try {
    if (upcomingResult.ok) upcoming = JSON.parse(upcomingResult.data);
  } catch {
    /* leave null */
  }
  try {
    if (overdueResult.ok) overdue = JSON.parse(overdueResult.data);
  } catch {
    /* leave null */
  }
...
app.get('/api/projects', async (_req, res) => {
  ...
  const results = await Promise.all(
    repos.map(async (repo) => {
      ...
      try {
        return { repo, runs: JSON.parse(result.data) };
      } catch {
        return { repo, error: 'Parse error' };
      }
    }),
  );
